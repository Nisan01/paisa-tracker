  ...
  return (
    <section id="pricing" className="px-6 md:px-12 py-10 md:py-8">
      <div className="max-w-6xl mx-auto">
        <motion.p
          {...fadeUp(0)}
          className="text-xs tracking-[3px] uppercase text-muted-foreground text-center md:text-left"
        >
          Pricing
        </motion.p>
        <motion.h2
          {...fadeUp(0.1)}
          className="text-3xl md:text-7xl font-medium tracking-[-1.2px] md:tracking-[-1.5px] mt-4 max-w-3xl text-center md:text-left"
        >
          Simple, <span className="font-serif italic font-normal">honest</span> pricing
        </motion.h2>
        <motion.p
          {...fadeUp(0.2)}
          className="text-muted-foreground text-base max-w-xl mt-4 text-center md:text-left"
        >
          No hidden fees. No surprises. Just powerful financial tracking that works for you.
        </motion.p>

        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 mt-10 md:mt-12">
          {plans.map((plan, i) => (
            <motion.div
              key={plan.name}
              {...fadeUp(0.1 + i * 0.1)}
            ...
            >
              ...
              <ul className="mt-5 space-y-4 h-[20vh]">
              ...
              <a
                href={plan.highlighted ? "#footer" : undefined}
                aria-disabled={!plan.highlighted}
                className={`w-full mt-12 py-2.5 rounded-lg text-xs font-semibold transition-all text-center inline-flex items-center justify-center ${
                  plan.highlighted
                    ? "bg-green-700 text-foreground hover:scale-[1.02] active:scale-[0.98]"
                    : "bg-muted/30 text-muted-foreground/50 cursor-not-allowed pointer-events-none"
                }`}
              >
